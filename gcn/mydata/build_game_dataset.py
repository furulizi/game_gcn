...
raw_x = pd.read_csv('ind.game.x.csv', sep=' ', header=None)
...
raw_ally = pd.read_csv('ind.game.ally.csv', sep=',', header=None)
raw_ally.columns = ['userid','tag']


ally = []
for i in range(trainset_size):
    one_hot = [0 for l in range(2)]
    label_index = raw_ally.iat[i, 1]
    if label_index > -1 :
...
y=[]
raw_y =pd.read_csv('ind.game.y.csv', sep=',', header=None)
raw_y.columns = ['userid', 'tag']
for i in range(trainset_size-unlabelset_size):
    one_hot = [0 for l in range(2)]
    label_index = raw_y.iat[i, 1]
    if label_index > -1 :
        one_hot[label_index] = 1
    y.append(one_hot)
y = np.array(y)
...
raw_ty = pd.read_csv('ind.game.ty.csv', sep=',', header=None)
raw_ty.columns = ['userid','tag']
ty = []
for i in range(testset_size):
    one_hot = [0 for l in range(2)]
    label_index = raw_ty.iat[i, 1]
    if label_index > -1:
        one_hot[label_index] = 1
    ty.append(one_hot)
ty = np.array(ty)
...
raw_graph = pd.read_csv('ind.game.graph.csv', sep=',', header=None)
...
with open('../data/ind.game.x','wb') as f:
    pickle.dump(x, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.y','wb') as f:
    pickle.dump(y, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.tx','wb') as f:
    pickle.dump(tx, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.ty','wb') as f:
    pickle.dump(ty, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.allx','wb') as f:
    pickle.dump(allx, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.ally','wb') as f:
    pickle.dump(ally, f, pickle.HIGHEST_PROTOCOL)
with open('../data/ind.game.graph','wb') as f:
